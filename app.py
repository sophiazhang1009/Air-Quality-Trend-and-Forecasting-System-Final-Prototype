...
air_img = load_image("data/air_image.jpg")
air_img2 = load_image("data/air_image_2.jpg")

@st.cache_data
def load_data():
    return pd.read_csv(
        "data/air_quality.csv",
        usecols=["City or Locality", "Version of the database", "PM2.5 (μg/m3)", "PM10 (μg/m3)", "NO2 (μg/m3)"]
    )
...
enough_data = cities_enough_data(df, pollutant)
...
@st.cache_data
def predict_pollutant(X, y, future_year):
    model = LinearRegression()
    model.fit(X, y)
    predicted = model.predict([[future_year + 4]])[0]
    return max(0, predicted)

# Then just call it:
predicted_value = predict_pollutant(X, y, future_year)
...
if pollutant == "PM2.5 (μg/m3)":
    if predicted_value == 5:
        ...
        with st.expander("Health Effects of PM2.5"):
            ...
            with col1:
                st.image(air_img, caption="Respiratory System Affected by Air Pollution", width=300)
            with col2:
...
if pollutant == "PM10 (μg/m3)":
    if predicted_value == 15:
        ...
        with st.expander("Health Effects of PM10"):
            ...
            col1, col2 = st.columns([1, 1])
            with col1:
                st.image(air_img, caption="Respiratory System Affected by Particulate Matter", width=300)
            with col2:
...
if pollutant == "NO2 (μg/m3)":
    if predicted_value == 10:
        ...
        with st.expander("Health Effects of NO2"):
            ...
            col1, col2 = st.columns([1, 1])
            with col1:
                st.image(air_img2, caption="Unaffected Airway versus Asthmatic Airway", width=300)
            with col2:
